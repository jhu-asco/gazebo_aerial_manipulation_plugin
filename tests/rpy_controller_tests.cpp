...
public:
  RpyControllerTests(): p_gains_(1.0, 1.0, 1.0)
...
TEST_F(RpyControllerTests, testConverged) {
  ...
  Vector3 body_torque = controller_.update(rpy_command, orientation, 0.01);
...
TEST_F(RpyControllerTests, testdT) {
  ...
  Quaternion orientation(Vector3::Zero);
  Vector3 omega(0,0,0);
  Vector3 body_torque = controller_.update(error_rpy, 100, 0.01);
  Vector3 updated_body_torque = controller_.update(error_rpy, 100, 0.01001);
  ...
  updated_body_torque = controller_.update(error_rpy, 100, 2.0);
...
TEST_F(RpyControllerTests, testRunning) {
  ...
  for(int i = 0; i < 100; i++) {
    ...
    body_torque = controller_.update(rpy_command, orientation, omega, time);
  ...
  ASSERT_NEAR(rpy.x, rpy_desired.x, 1e-2);
  ASSERT_NEAR(rpy.y, rpy_desired.y, 1e-2);
  ASSERT_NEAR(rpy.z, rpy_desired.z, 1e-2);
}
